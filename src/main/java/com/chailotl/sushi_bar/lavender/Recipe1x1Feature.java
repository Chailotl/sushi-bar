...
public class Recipe1x1Feature implements MarkdownFeature
{
	...
	public static final RecipeFeature.RecipePreviewBuilder<CraftingRecipe> CRAFTING_2X2_PREVIEW_BUILDER = new RecipeFeature.RecipePreviewBuilder<>()
	{
		...
		{
			var recipeComponent = componentSource.template(UIModelLoader.get(new Identifier("sushi_bar:book_components")), ParentComponent.class, "crafting-1x1-recipe");

			populateIngredientsGrid(recipe, recipe.getIngredients(), recipeComponent.childById(ParentComponent.class, "input-grid"), 1, 1);
	...
	public Recipe1x1Feature(BookCompiler.ComponentSource bookComponentSource, @Nullable Map<RecipeType<?>, RecipeFeature.RecipePreviewBuilder<?>> previewBuilders)
	{
	...
	public void registerTokens(MarkdownFeature.TokenRegistrar registrar)
	{
		registrar.registerToken((nibbler, tokens) -> {
			if (!nibbler.tryConsume("<recipe_1x1;")) { return false; }
	...
	private class RecipeNode extends Parser.Node
	{
		...
		{
			var previewBuilder = (RecipeFeature.RecipePreviewBuilder) previewBuilders.get(recipe.getType());
			...
			else
			{
				((OwoUICompiler) compiler).visitComponent(
					Containers.verticalFlow(Sizing.fill(100), Sizing.content())
						.child(Components.label(Text.literal("No preview builder registered for recipe type '" + Registries.RECIPE_TYPE.getId(recipe.getType()) + "'"))
